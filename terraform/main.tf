module "service" {
  source = "/home/lu/Documents/projetos/linuxtips-aws-containers-ecs-service-module"
  region = var.region
  ...
  service_task_execution_role = aws_iam_role.main
  ...
  private_subnets = [
    data.aws_ssm_parameter.priv_subnet_1.value,
    data.aws_ssm_parameter.priv_subnet_2.value,
  ...
  scaling_tracking_requests = var.scale_tracking_requests
